...
def create_mtcnn_net(image, mini_face, device, p_model_path=None, r_model_path=None, o_model_path=None):
    ...
    if p_model_path is not None:
        pnet = PNet().to(device)
        pnet.load_state_dict(torch.load(p_model_path, map_location=lambda storage, loc: storage, weights_only=True))
        pnet.eval()
    ...
    if r_model_path is not None:
        rnet = RNet().to(device)
        rnet.load_state_dict(torch.load(r_model_path, map_location=lambda storage, loc: storage, weights_only=True))
        rnet.eval()
    ...
    if o_model_path is not None:
        onet = ONet().to(device)
        onet.load_state_dict(torch.load(o_model_path, map_location=lambda storage, loc: storage, weights_only=True))
        onet.eval()
...
if __name__ == '__main__':
    ...
    "test image path", default="E:\\AI\\mobilefacenet_02\\MobileFaceNet_Tutorial_Pytorch-master\\MTCNN\\images\\office1.jpg", type=str)
    ...
    bboxes, landmarks = create_mtcnn_net(image, args.mini_face, device, p_model_path='E:\\AI\\mobilefacenet_02\\MobileFaceNet_Tutorial_Pytorch-master\\MTCNN\\weights\\pnet_Weights', r_model_path='E:\\AI\\mobilefacenet_02\\MobileFaceNet_Tutorial_Pytorch-master\\MTCNN\\weights\\rnet_Weights', o_model_path='E:\\AI\\mobilefacenet_02\\MobileFaceNet_Tutorial_Pytorch-master\\MTCNN\\weights\\onet_Weights')
